...
namespace CNO.BPA.ScanPop
{
    public class ScanPlusTaskEvents : IScanPlusTaskEvents
    {
                        ...
                        case "STANDARD_MDF":
                            {
                                ...
                                string scanOperator = "";
                                foreach (IWorkflowStep scanStep in taskInfo.Application.CurrentStep.BatchProcess.WorkflowSteps)
                                {
                                    if (scanStep.Name.ToUpper() == "SCAN")
                                    {
                                        scanOperator = taskInfo.Application.CurrentStep.BatchProcess.Tree.Values(scanStep).GetString("ScanOperator", "");
                                        break;
                                    }
                                }

                                taskInfo.Application.CurrentStep.BatchProcess.Tree.Values(wfStep).SetString("SCAN_OPERATOR", ParseScanOperator(scanOperator));                                
        ...
        private string ParseScanOperator(string scanOperator)
        {
            ...
            int index = scanOperator.IndexOf("\\");
            return scanOperator.Substring(index + 1);
